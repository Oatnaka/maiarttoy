...
BOOTSTRAP_CLASSES = 'form-control form-control-lg rounded-3'
...
class ProductForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'name': forms.TextInput(attrs={'class': BOOTSTRAP_CLASSES, 'placeholder': 'ชื่อสินค้า'}),
            'description': forms.Textarea(attrs={'rows': 4, 'class': TEXTAREA_CLASSES, 'placeholder': 'รายละเอียดสินค้า'}),
            'price': forms.NumberInput(attrs={'class': BOOTSTRAP_CLASSES, 'placeholder': 'ราคา (บาท)', 'step': '0.01'}),
            'stock': forms.NumberInput(attrs={'class': BOOTSTRAP_CLASSES, 'placeholder': 'จำนวนสต็อก'}),
...
class CustomAuthenticationForm(AuthenticationForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # ตั้งค่า Class ให้กับฟิลด์ username และ password
